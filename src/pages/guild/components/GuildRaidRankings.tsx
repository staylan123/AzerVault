...
const GuildRaidRankings = ({ raidRankings }: GuildRaidRankingsProps) => {
  ...
  const hasAnyRanking = entries.some(([, ranks]) =>
    (["mythic", "heroic", "normal"] as const).some(diff => {
      const r = ranks[diff]
      return r && (r.world !== 0 || r.region !== 0 || r.realm !== 0)
    }),
  )
  ...
  return (
    <div>
      ...
      {!hasAnyRanking ? (
        <p className="text-sm text-text-muted">No ranking data available for this guild.</p>
      ) : (
